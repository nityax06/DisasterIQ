...
export default function ResourcePanel() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem("disasteriq-resource-requests");
    if (saved) {
      setRequested(JSON.parse(saved));
    }

    function requestAllShortages() {
      const shortageNames = resources
        .filter((resource) => resource.required - resource.available > 0)
        .map((resource) => resource.name);

      const merged = Array.from(new Set([...requested, ...shortageNames]));

      setRequested(merged);
      localStorage.setItem("disasteriq-resource-requests", JSON.stringify(merged));

      addNotification(
        "Bulk refill request created",
        `${shortageNames.length} shortage resources were added to refill queue.`,
        "resource"
      );

      showToast("Bulk refill request created");
    }

    window.addEventListener("dashboard-resource-refill", requestAllShortages);

    return () => {
      window.removeEventListener("dashboard-resource-refill", requestAllShortages);
    };
  }, [requested]);

  function requestRefill(resourceName: string) {
    if (requested.includes(resourceName)) return;

    const updated = [...requested, resourceName];

    setRequested(updated);
    localStorage.setItem("disasteriq-resource-requests", JSON.stringify(updated));

    addNotification(
      "Resource refill requested",
      `${resourceName} refill request was submitted.`,
      "resource"
    );

    showToast(`${resourceName} refill requested`);
  }

  const shortageCount = resources.filter(
    (resource) => resource.required - resource.available > 0
  ).length;
            ...
            {resources.map((resource) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-3 py-2 text-slate-300">{level.eta}</td>
