...
export default function StatusWorkflowBoard({
  ...
  const columns = [
    { title: "Reported", items: incidents.slice(0, 2), color: "border-slate-500/20" },
    { title: "Investigating", items: incidents.slice(2, 3), color: "border-yellow-500/20" },
    { title: "Responding", items: incidents.slice(3, 4), color: "border-blue-500/20" },
    { title: "Resolved", items: incidents.slice(4, 5), color: "border-green-500/20" },
  ];
    ...
    <div className="rounded-xl border border-white/10 bg-white/[0.03] p-4 transition hover:bg-white/[0.05]">
      ...
      <div className="grid grid-cols-4 gap-2">
        {columns.map((column) => (
          <div
            key={column.title}
            className={`min-h-44 rounded-lg border ${column.color} bg-black/30 p-2 transition hover:bg-white/[0.04]`}
          >
            ...
            <div className="space-y-2">
              {column.items.length === 0 ? (
                ...
                column.items.map((incident) => (
                  <div
                    key={incident.id}
                    className="rounded-md border border-white/10 bg-white/[0.04] p-2 text-[11px] transition hover:border-blue-500/30 hover:bg-blue-500/10"
                  >
